<?php include "include/headerlogin.php" ?>
<?php include "include/navlogin.php" ?>
<?php include "include/db.php" ?>
<br> &nbsp; <br>
<!-- ======= Services Section ======= -->
<section id="services" class="services section-bg">
  <div class="container" data-aos="fade-up">

    <div class="section-title">
      <h2>Services</h2>
      <p>Layanan yang kami tawarkan</p>
    </div>

    <div class="row">
      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-xl-0" data-aos="zoom-in" data-aos-delay="100">
        <div class="icon-box">
          <div class="icon"><i class="bx bxl-dribbble"></i></div>
          <h4><a href="/tukangku/perbaikan/AirConditioner.php">Air Conditioner</a></h4>
          <p>Perbaikan dan pemasangan AC di rumah anda</p>
        </div>
      </div>

      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-4 mt-md-0" data-aos="zoom-in" data-aos-delay="200">
        <div class="icon-box">
          <div class="icon"><i class="bx bx-file"></i></div>
          <h4><a href="/tukangku/perbaikan/wallpaper.php">Wallpaper</a></h4>
          <p>Pemasangan wallpaper untuk memperindah hunian anda</p>
        </div>
      </div>


      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-4 mt-xl-0" data-aos="zoom-in" data-aos-delay="400">
        <div class="icon-box">
          <div class="icon"><i class="bx bx-layer"></i></div>
          <h4><a href="/tukangku/perbaikan/listrik.php">Listrik</a></h4>
          <p>Perbaikan dan pemasangan jaringan listrik dirumah anda</p>
        </div>
      </div>

      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-4 mt-xl-0" data-aos="zoom-in" data-aos-delay="400">
        <div class="icon-box">
          <div class="icon"><i class="bx bx-layer"></i></div>
          <h4><a href="/tukangku/perbaikan/Cat.php">Cat</a></h4>
          <p>Pengecatan sesuai dengan warna yang anda tentukan</p>
        </div>
      </div>
      <br>
      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-4 mt-xl-0" data-aos="zoom-in" data-aos-delay="400">
        <div class="icon-box">
          <div class="icon"><i class="bx bx-layer"></i></div>
          <h4><a href="/tukangku/perbaikan/leding.php">Leding</a></h4>
          <p>Perbaikan perpipaan dan sarana sanitasi dirumah anda.</p>
        </div>
      </div>

      <div class="col-xl-3 col-md-6 d-flex align-items-stretch mt-4 mt-xl-0" data-aos="zoom-in" data-aos-delay="400">
        <div class="icon-box">
          <div class="icon"><i class="bx bx-layer"></i></div>
          <h4><a href="/tukangku/perbaikan/furniture.php">furniture </a></h4>
          <p>Perbaikan perpipaan dan sarana sanitasi dirumah anda.</p>
        </div>
      </div>

    </div>

  </div>
</section><!-- End Services Section -->
<?php include "include/footer.php" ?>